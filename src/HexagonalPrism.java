...
public class HexagonalPrism implements GLEventListener{
    ...
    // Define the 12 vertices of the hexagonal prism.
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        // Draw the first hexagonal face
        gl.glBegin(GL2.GL_POLYGON);
        gl.glColor3f(1.0f, 0.0f, 0.0f); // sets the color
        //first six vertices of the array
        for (int i = 0; i < 6; i++) {
            gl.glVertex3fv(vertices[i], 0);
        }
        //completes the specification of the polygon
        gl.glEnd();
        // Draw the second hexagonal face
        gl.glBegin(GL2.GL_POLYGON);
        //second six vertices of the array which means altogether 12 vertices
        gl.glColor3f(0.0f, 1.0f, 0.0f); // sets the color
        for (int i = 6; i < 12; i++) {
        ...
        //completes the specification of the polygon
        gl.glEnd();
        //connects the vertices of the top and bottom and draws the six rectangular faces of the prism. These faces are drawn as the quadrilaterals.
        gl.glBegin(GL2.GL_QUADS);
        gl.glColor3f(0.0f, 0.0f, 1.0f); // set the color
        for (int i = 0; i < 6; i++) {
            gl.glVertex3fv(vertices[i], 0);// set to the first vertex of the quadrilateral.
            gl.glVertex3fv(vertices[(i + 1) % 6], 0);// set to the second vertex of the quadrilateral.
            gl.glVertex3fv(vertices[(i + 1) % 6 + 6], 0);// set to the third vertex of the quadrilateral
            gl.glVertex3fv(vertices[i + 6], 0);// set to the fourth vertex of the quadrilateral.
        }
    ...
    public static void main(String[] args){
        ...
        //calling the HexagonalPrism class
        HexagonalPrism hexagonalPrism = new HexagonalPrism();
        //gets the OpenGL events such as display, dispose, init and reshape
        glCanvas.addGLEventListener(hexagonalPrism);
        //sets the canvas size
        glCanvas.setSize(400, 400);
        //creates a Java Frame
        final JFrame frame = new JFrame("3D Hexagonal Prism");
        ...
        JLabel titleLabel = new JLabel("3D Hexagonal Prism Visualization", SwingConstants.CENTER);
        titleLabel.setFont(new Font("Times New Roman", Font.BOLD, 30));
        titleLabel.setForeground(Color.WHITE);
        titlePanel.add(titleLabel, BorderLayout.CENTER);

        // create a button to the hexagon screen
        JButton HexagonButton = new JButton("Back");
        HexagonButton.setBackground(new Color(48, 57, 82));
        HexagonButton.setForeground(Color.WHITE);
        HexagonButton.setToolTipText("Back to Hexagon Screen");
        HexagonButton.addActionListener(event -> {
        ...
        titlePanel.add(HexagonButton, BorderLayout.WEST);
